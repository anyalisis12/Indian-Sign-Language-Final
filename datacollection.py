...
def capture_gesture_images(gesture_name, max_images=200, roi_coords=(100, 100, 300, 300), base_dir='dataSet'):
    ...
    if image_count == max_images:
        print(f"Successfully collected {max_images} images for gesture '{gesture_name}'.")
        log_event(f"Completed collection of {max_images} images for gesture '{gesture_name}'.")
    else:
        print(f"Collection terminated after {image_count} images.")
        log_event(f"Terminated collection after {image_count} images for gesture '{gesture_name}'.")
...
def main():
    ...
    max_images = int(input("Enter the number of images to capture (default 200): ") or 200)

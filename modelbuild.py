...
def main():
    ...
    print(f"Input Shape: {input_shape}, Dropout Rate: {dropout_rate}, Learning Rate: {learning_rate}")

    model = create_cnn_model(input_shape=input_shape, num_classes=num_classes, dropout_rate=dropout_rate)

    compile_model(model, optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy'])
